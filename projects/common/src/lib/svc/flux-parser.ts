...
export class FluxParser {
  ...
  //     if (item.data.SchemaType == 'incomming' && item.data.IncommingModuleID) schemaNode.IncommingModuleID = item.data.IncommingModuleID;
  ...
  //     if (item.data.DisableSchemaEdit) schemaNode.DisableSchemaEdit = item.data.DisableSchemaEdit;
  ...
  //         if (func.ExtraData && func.ExtraData.HasErrors) returnObj.HasErrors = true;
  ...
  //     if (item.data.JoinFunctionNeeded) returnObj.HasErrors = true;

  //     if (item.data.Timestamp) schemaNode.Timestamp = item.data.Timestamp;

  //     if (item.data.Groups) schemaNode.Groups = item.data.Groups;
  ...
  //     if (edges.length == 0 && item.data.SchemaType == 'outgoing') returnObj.HasErrors = true;
  ...
  //     if (item.data.SchemaType == 'incomming' && !item.data.IncommingModuleID) returnObj.HasErrors = true;

  //     returnObj.SchemaNodes.push(schemaNode);

  //     item.getPorts().forEach(function(port) {
  //       if (port.data.MappingFunctionNeeded) returnObj.HasErrors = true;
  ...
  //     if (node.data.SchemaType == 'incomming') portType = 'source';
  //     else portType = 'target';
  ...
  //     keys.forEach(key => {
  //       if (schema.properties[key].id == id) found = schema.properties[key];
  ...
  //       if (schemaList && schemaList.length > 0) node['Schema'] = JSON.parse(JSON.stringify(schemaList[0]));

  //       node.FilterFunctionError = false;

  //       if (item.IncommingModuleID) node.IncommingModuleID = item.IncommingModuleID;

  //       if (item.OutgoingModuleIDs) node.OutgoingModuleIDs = item.OutgoingModuleIDs;

  //       if (item.DisableSchemaEdit) node.DisableSchemaEdit = item.DisableSchemaEdit;
  ...
  //       if (item.Groups) node.Groups = item.Groups;

  //       if (item.Timestamp) node.Timestamp = item.Timestamp;
  ...
  //         if (!func.Type) func.Type = 'mapping';
  ...
  //           if (!node.data.Functions) node.data.Functions = new Array<SchemaFunction>();
  ...
  //           if (func.ExtraData.HasErrors) port.data.MappingFunctionError = true;
  //         } else if (func.Type == 'filter') {
  //           node.data.Functions.push(newFunc);
  //           if (func.ExtraData.HasErrors) node.data.FilterFunctionError = true;
  //         } else if (func.Type == 'join') {
  //           node.data.JoinRelationships[func.ResultPropertyID].Functions.push(newFunc);
  //           if (func.ExtraData.HasErrors) node.data.JoinRelationships[func.ResultPropertyID].JoinFunctionError = true;
  //         }
  //       });
  //     }
  ...
  //                 if (needed && !params.IsJoinPresent(node)) node.data.JoinFunctionNeeded = true;
  //                 if (needed) node.data.ShowJoinLink = true;
  //               });
  //             }
  ...
  //               if (isMapNeeded && !edge.target.data.SchemaFunctionsReturnSource) edge.target.data.MappingFunctionNeeded = true;
  //             }
  //           });
